...
At some point in our careers, we will likely need to implement some code

```{r}
team_abbrs <- str_sub(team_urls, start = 46, end = 48)
...
for(team in team_abbrs){
  for(player in player_urls[[team]]){
...
```{r}
library(foreach)
rush_receive_stats.do <- foreach(x = team_urls,
        .packages = 'dplyr') %do% {
  x %>%
    get_team_stats() %>%
    clean_stats()
} %>%
  set_names(., str_sub(team_urls, start = 46, end = 48)) %>%
  bind_rows(., .id = 'team')
```
